...
from openhands.agenthub.dspy_codeact_agent.codeact_agent import DSPyCodeActAgent
...
@pytest.fixture
def agent() -> DSPyCodeActAgent:
    agent = DSPyCodeActAgent(llm=LLM(LLMConfig()), config=AgentConfig())
    agent.llm = Mock()
...
def test_cmd_output_observation_message(agent: DSPyCodeActAgent):
...
def test_ipython_run_cell_observation_message(agent: DSPyCodeActAgent):
    agent.config.function_calling = False
...
def test_agent_delegate_observation_message(agent: DSPyCodeActAgent):
    agent.config.function_calling = False
...
def test_error_observation_message(agent: DSPyCodeActAgent):
    agent.config.function_calling = False
...
def test_unknown_observation_message(agent: DSPyCodeActAgent):
    obs = Mock()
